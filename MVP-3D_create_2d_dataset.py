...
def get_config():
    ...
    if config.compile_JSON: 
        ...
        config.JSON_dataset_outpath = "{}/data/body/2d_proposals/all_tasks_2D_kpts.pickle".format(config.root_dir,)
    return config
            ...
            if S_id in config.subjs_to_get_preds:
                for ch in config.chs:
                    ...
                    if S_id in subjects_new_sys:
                        ...
                        file_subpath += 'LNR616X_ch{}_main_{}.avi'.format(ch[-1], new_sys_vid_suffixes[S_id][task])
                    ...
                    out_path = config.dataset_path + str(id) + '/' + task + '/'
    ...
    for task in config.updrs_task:
        ...
        for S_id, id in subjects_ALL_id_dict.items():
            if S_id in config.subjs_to_compile:
                print("\n--- {}: ---".format(S_id))
                # Construct paths
                in_json_path = config.dataset_path + str(id) + '/' + task + '/'

                # FOR NOW MUST USE A PAIR OF CHANNELS
                kpts_dict = filter_alphapose_results(in_json_path, S_id, task, config.chs, kpts_dict)
        # Print quick summary of the dataset
        print("\n --- Summary of task {}---".format(task))
        print("  Total num subjects: ", len(kpts_dict.keys()))
        print("  Total num videos: ", sum([len(kpts_dict[subj][task].keys()) for subj in kpts_dict.keys()]))
        print("  Total num new sys (training) frames: ", sum([kpts_dict[subj][task]['pos'][0].shape[0] for subj in kpts_dict.keys() if subj in subjects_new_sys]))
        print("  Avg num new sys (training) frames: ", np.mean([kpts_dict[subj][task]['pos'][0].shape[0] for subj in kpts_dict.keys() if subj in subjects_new_sys]))
        print("  Total num old sys frames: ", sum([kpts_dict[subj][task]['pos'][0].shape[0] for subj in kpts_dict.keys() if subj not in subjects_new_sys]))
        print("  Avg num old sys frames: ", np.mean([kpts_dict[subj][task]['pos'][0].shape[0] for subj in kpts_dict.keys() if subj not in subjects_new_sys]))
