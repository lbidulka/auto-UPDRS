...
def body_tasks(input_args):
    ...
    feat_accs = [np.round(accuracy_score(info.Y_true, np.round(pd_indicators[i])), 4) for i in range(len(info.clinical_gait_feat_names))]
    print("\n individ. feat accs:\n", info.clinical_gait_feat_acronyms, "\n", feat_accs)
    group_feat_accs = [np.round(accuracy_score(info.Y_true, np.round(pd_indicators_grouped[i])), 4) for i in range(8)]
