...
def main():
    ...
    for S_id, id in subjects_ALL_id_dict.items():
        if S_id in ['S02', 'S25', 'S26', 'S27', 'S28', 'S29']: # ['S25',]: #TEMP FOR TESTING
            print("Trying ", S_id,)
            # Construct paths
            in_json_path = input_args.dataset_path + str(id) + '/' + task + '/'
            ch1_in_json_pth =  in_json_path + 'CH' + chs[0] + "_alphapose-results.json"
            ch2_in_json_pth =  in_json_path + 'CH' + chs[1] + "_alphapose-results.json"
            ...
            if not os.path.exists(ch1_in_json_pth):
                ...
                kpts_dict = MOHSEN_filter_alphapose_results(in_json_path, S_id, task, kpts_dict)
